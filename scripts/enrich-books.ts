/* ============================================================
   ...
   sourced fields — img / sub / pub / rn / rc / rsrc — into the generated
...
import { buildQueryUrl, pickMatch, mapVolume, type BookMeta, type GVolume } from '../src/lib/gbooks';
...
/** resolve one book: Google first (if keyed), then Open Library */
async function resolveOne(title: string, author: string): Promise<BookMeta | null> {
  if (KEY) {
    try {
      const res = await fetch(buildQueryUrl(title, author, KEY));
      if (res.ok) {
        const data = (await res.json()) as { items?: GVolume[] };
        const m = pickMatch(data.items ?? [], title, author);
        if (m) return mapVolume(m);
      }
    } catch {
...
async function run() {
  ...
  for (const id of ids) {
    const b = BOOKS[id];
    try {
      const m = await resolveOne(b.t, b.a);
      if (m) {
        const e = sidecar(m);
  ...
  const header = `/* ============================================================
   ...
   catalog book and writes only the sourced fields — img/sub/pub/rn/rc/rsrc —
