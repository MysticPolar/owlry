...
export const makeBook = async file => {
    ...
    if (file.isDirectory) {
        ...
        // [owlry patch] PDFs are rendered by the app's own pdf.js engine (PdfView),
        // never by foliate — dropping this branch avoids vendoring foliate's 13 MB pdfjs.
        throw new UnsupportedTypeError('PDF is handled by the app’s own engine, not foliate')
    }
    else {
