...
function offlineTurn(text: string, ctx: TurnContext): { reply: OwlReply; session: OwlSession } {
  const session: OwlSession = { ...ctx.session, wxKey: ctx.wxKey, usedGuides: [...ctx.session.usedGuides] };
  return { reply: respond(text, session), session };
}

async function liveTurn(text: string, ctx: TurnContext): Promise<{ reply: OwlReply; session: OwlSession }> {
  const { data, error } = await supabase!.functions.invoke('owl-chat', {
    body: { message: text, client_day: localDay() },
  });
  if (error) throw error;
  const v = validateChatV2(data);
  if (!v.ok) throw new Error(`owl-chat contract: ${v.errors.join('; ')}`);
  return { reply: mapChatV2(v.res), session: ctx.session };
}

/** Resolve one owl turn (bubble + book pick + chips; no letter). */
export async function fetchOwlTurn(text: string, ctx: TurnContext): Promise<{ reply: OwlReply; session: OwlSession }> {
  if (!supabase) return offlineTurn(text, ctx);
  try {
